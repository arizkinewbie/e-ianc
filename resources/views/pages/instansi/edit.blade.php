...
<div class="row justify-content-center">
    <div class="col-md-9">
        <div class="card">
            <form action="{{ route('instansi.update') }}" method="post" autocomplete="off" enctype="multipart/form-data">
                @csrf
                <input type="hidden" name="id" value="{{ $data->ins_id }}">
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <select name="ins_type" id="ins_type" class="form-control select2">
                                    ...
                                    @foreach ($type as $t)
                                        <option value="{{ $t->ti_id }}" {{ (old('ins_type') == $t->ti_id) ? 'selected' : (($t->ti_id == $data->ins_type) ? 'selected' : '') }}>{{ $t->ti_name }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="ins_name" id="ins_name" class="form-control"
                                    value="{{ (old('ins_name')) ? old('ins_name') : $data->ins_name }}">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="ins_telp" id="ins_telp" class="form-control"
                                    value="{{ (old('ins_telp')) ? old('ins_telp') : $data->ins_telp }}" maxlength="15">
                            ...
                            <div class="form-group">
                                <div class="row">
                                    ...
                                    <div class="col-md-6">
                                        <input type="text" name="ins_code" id="ins_code" class="form-control" value="{{ (old('ins_code')) ? old('ins_code') : $data->ins_code }}">
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-md-6">
                                            <div class="form-check">
                                                <input class="form-check-input" type="checkbox" value="1" id="ins_bpjs" name="ins_bpjs" {{ (old('ins_bpjs') == '1') ? 'checked' : (($data->ins_bpjs == '1') ? 'checked' : '') }}>
                                ...
                                <div class="col-md-8">
                                    <div class="form-group">
                                        <div class="text-center">
                                            <img src="{{ asset('data/image/instansi/'. $data->ins_thumb) }}" id="image-preview" height="256">
                                        </div>
                                        <div class="custom-file">
                                            <input type="file" name="thumb" class="custom-file-input" id="thumb" onchange="previewImage();">
                                            <input type="hidden" name="old_thumb" value="{{ $data->ins_thumb }}">
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <select name="ins_province" id="ins_province" class="form-control select2">
                                    ...
                                    <option value="{{ $pro->kode }}" {{ ($pro->kode == old('ins_province')) ? 'selected' : (($pro->kode == $data->ins_province) ? 'selected' : '' ) }}>{{ $pro->nama }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="form-group">
                                ...
                                <select name="ins_district" id="ins_district" class="form-control select2">
                                    @php
                                        $kab = DB::table('sys_alamats')->where('kode', $data->ins_district)->get();
                                    @endphp
                            ...
                            <div class="form-group">
                                ...
                                <select name="ins_subdistrict" id="ins_subdistrict" class="form-control select2">
                                    @php
                                        $kab = DB::table('sys_alamats')->where('kode', $data->ins_subdistrict)->get();
                                    @endphp
                                    <option value="{{ $kab[0]->kode }}">{{ $kab[0]->nama }}</option>
                                </select>
                            </div>
                            <div class="form-group">
                                ...
                                <select name="ins_village" id="ins_village" class="form-control select2">
                                    @php
                                        $kab = DB::table('sys_alamats')->where('kode', $data->ins_village)->get();
                                    @endphp
                                    <option value="{{ $kab[0]->kode }}">{{ $kab[0]->nama }}</option>
                                </select>
                            </div>
                                ...
                                <div class="row">
                                    <div class="col-md-6">
                                        ...
                                        <select name="ins_rw" id="ins_rw" class="form-control select2">
                                            ...
                                            @for ($i = 1; $i < 100; $i++)
                                                <option value="{{ sprintf("%03s", ($i)) }}" {{ (sprintf("%03s", ($i)) == old('ins_rw')) ? 'selected' : ((sprintf("%03s", ($i)) == $data->ins_rw) ? 'selected' : '') }}>{{ sprintf("%03s", ($i)) }}</option>
                                            @endfor
                                        </select>
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <select name="ins_rt" id="ins_rt" class="form-control select2">
                                            <option value="x">-- PILIH RW --</option>
                                            @for ($i = 1; $i < 100; $i++)
                                                <option value="{{ sprintf("%03s", ($i)) }}" {{ (sprintf("%03s", ($i)) == old('ins_rt')) ? 'selected' : ((sprintf("%03s", ($i)) == $data->ins_rt) ? 'selected' : '') }}>{{ sprintf("%03s", ($i)) }}</option>
                                            @endfor
                                        </select>
                            ...
                            <div class="form-group">
                                ...
                                <textarea name="ins_address" id="ins_address" cols="30" rows="3"
                                    class="form-control">{{ (old('ins_address')) ? old('ins_address') : $data->ins_address }}</textarea>
                            ...
                            <div class="form-group">
                                <div class="row">
                                    ...
                                    <div class="col-md-8">
                                        <input type="text" name="ins_zip_code" id="ins_zip_code" class="form-control text-center" value="{{ (old('ins_zip_code')) ? old('ins_zip_code') : $data->ins_zip_code }}">
                                    </div>
                                </div>
                            </div>
                            <div class="form-group">
                                <label for="ins_status">Status</label>
                                <select name="ins_status" id="ins_status" class="form-control">
                                    <option value="0" {{ ($data->ins_status == '0') ? 'selected' : '' }}>Blocked</option>
                                    <option value="1" {{ ($data->ins_status == '1') ? 'selected' : '' }}>Active</option>
                                </select>
                            </div>
                        </div>
                    </div>
