                ...
                @foreach ($data as $d)
                    <tr>
                        ...
                        <td class="text-center">
                            {{ $d->det_pn_year }} - {{ DB::table('selec_months')->where('mon_id', $d->det_pn_month)->value('mon_name') }}
                        </td>
                        <td></td>
                        <td class="text-center">
                            <a href="{{ route('pn.edit', Crypt::encrypt($d->det_pn_id)) }}">
                                <button class="btn btn-sm btn-warning" title="edit">
                                    <i class="fa fa-edit"></i>
                                </button>
                            </a>
                            &nbsp;|&nbsp;
                            <form action="{{ route('pn.destroy') }}" method="post" class="d-inline">
                                @csrf
                                <input type="hidden" name="id" value="{{ Crypt::encrypt($d->det_pn_id) }}">
...
<div class="modal fade" id="exampleModalCenter" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle"
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <form action="{{ route('pn.cek') }}" name="formreg" method="post" autocomplete="off" onsubmit="return validat();">
                @csrf
...
<script>
    window.open("{{ route('pn.report', ['month' => session()->get('rmonth'), 'year' => session()->get('ryear')]) }}", "_blank", "toolbar=yes,scrollbars=yes,resizable=yes,top=100,left=100,width=1270,height=720");
</script>
@endif
@endsection
